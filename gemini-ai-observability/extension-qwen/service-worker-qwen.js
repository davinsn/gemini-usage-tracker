...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (!message) {
    console.error("[qwen-obs] REJECTED: message is undefined");
  ...
  if (message.type !== "QWEN_USAGE_EVENT") {
    console.warn("[qwen-obs] Ignoring unrelated message:", message.type);
  ...
  if (!event) {
    console.error("[qwen-obs] REJECTED: event is missing");
  ...
  const detectedEmail =
    event.email || event.employeeEmail || message.employeeEmail || null;

  console.log("[qwen-obs] DETECTED QWEN EMAIL:", detectedEmail);

  if (!detectedEmail || typeof detectedEmail !== "string") {
    console.error("[qwen-obs] REJECTED: no employee email");
  ...
  if (!eventType) {
    console.error("[qwen-obs] REJECTED: event_type is missing");
  ...
  console.log("[qwen-obs] Employee Email:", payload.email);
  ...
  (async () => {
    try {
      ...
      console.log("[qwen-obs] BACKEND HTTP STATUS:", response.status);
      ...
      if (contentType.includes("application/json")) {
        try {
          ...
          console.error("[qwen-obs] JSON PARSE ERROR:", error);
        }
      } else {
        try {
      ...
      console.log("[qwen-obs] BACKEND RESPONSE:", data);
      ...
      if (!response.ok) {
        console.error("[qwen-obs] BACKEND REJECTED EVENT");

        console.error("[qwen-obs] Status:", response.status);

        console.error("[qwen-obs] Response:", data);
      ...
      console.log("[qwen-obs] EVENT SUCCESSFULLY SENT");

      console.log("[qwen-obs] Employee Email:", detectedEmail);

      console.log("[qwen-obs] Provider:", provider);

      console.log("[qwen-obs] Product:", product);

      console.log("[qwen-obs] Event:", eventType);

      console.log("[qwen-obs] Model:", event.model);

      console.log("[qwen-obs] Interaction:", event.interaction_id);
      ...
      console.error("[qwen-obs] BACKEND FETCH ERROR:", error);
